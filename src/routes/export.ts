...
import MarkdownForDevice from '../service/export/markdownfordevice'
...
router.get('/export/markdownfordevice', async ctx => {
  const repoId = +ctx.query.id
  if (
    !(await AccessUtils.canUserAccess(
      ACCESS_TYPE.REPOSITORY_GET,
  ...
  ctx.body = await MarkdownForDevice.export(repoId, ctx.query.origin)
})
